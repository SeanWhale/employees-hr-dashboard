...
try:
    from core.config import DATASET_MAX_DATE, DB_PATH, PQ_DIR
except ImportError:
    from backend.core.config import DATASET_MAX_DATE, DB_PATH, PQ_DIR
...
def get_parquet_path(pattern: str) -> str:
    ...
    return os.path.join(PQ_DIR, pattern).replace("\\", "/")
